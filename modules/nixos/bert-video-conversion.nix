...
let
  cfg = config.services.bert-video-conversion;

  # Fetch script: Query GraphQL, download first unconverted video, start convert service
  fetchScript = pkgs.writeShellScript "bert-video-fetch.sh" ''
    set -euo pipefail

    # Check if other services are running
    if systemctl is-active --quiet bert-video-convert.service || systemctl is-active --quiet bert-video-upload.service; then
      echo "[INFO] Other services are running, exiting"
      exit 0
    fi
...
QUERY_END
    ...
    echo "[INFO] Download complete, starting convert service"
    systemctl start bert-video-convert.service
  '';
  ...
  convertScript = pkgs.writeShellScript "bert-video-convert.sh" ''
    ...
    # Check if other services are running
    if systemctl is-active --quiet bert-video-fetch.service || systemctl is-active --quiet bert-video-upload.service; then
      echo "[INFO] Other services are running, exiting"
      exit 0
    fi
    ...
    echo "[INFO] All conversions complete, starting upload service"
    systemctl start bert-video-upload.service
  '';
  ...
  uploadScript = pkgs.writeShellScript "bert-video-upload.sh" ''
    ...
    # Check if other services are running
    if systemctl is-active --quiet bert-video-fetch.service || systemctl is-active --quiet bert-video-convert.service; then
      echo "[INFO] Other services are running, exiting"
      exit 0
    fi
    ...
    echo "[INFO] Starting fetch service to get next video"
    systemctl start bert-video-fetch.service
  '';

in
{
  options.services.bert-video-conversion = {
    ...
    environmentFile = mkOption {
      ...
      description = ''
        ...
        ''';
        services.bert-video-conversion.environmentFile =
          config.sops.templates."bert-video-conversion.env".path;
        ```
      '';
    };
    ...
    stateDirectory = mkOption {
      type = str;
      default = "/var/lib/bert-video-conversion";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.targets.bert-video-conversion = {
      description = "Bert Video Conversion Target";
      wants = [
        "bert-video-fetch.service"
        "bert-video-convert.service"
        "bert-video-upload.service"
      ];
    };
  };
}
