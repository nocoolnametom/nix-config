    ...
    deluge = lib.lists.optionals config.services.deluge.web.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString configVars.networking.ports.tcp.delugeweb}/"
          else
    ...
    flood = lib.lists.optionals config.services.flood.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString config.services.flood.port}/"
          else
    ...
    ombi = lib.lists.optionals config.services.ombi.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString config.services.ombi.port}/"
          else
    ...
    navidrome = lib.lists.optionals config.services.navidrome.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString config.services.navidrome.settings.Port}/"
          else
    ...
    nzbget = lib.lists.optionals config.services.nzbget.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString configVars.networking.ports.tcp.nzbget}/"
          else
    ...
    stashapp = lib.lists.optionals config.services.stashapp.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString config.services.stashapp.port}/"
          else
            "https://${configVars.networking.subdomains.stash}.${configVars.domain}";
        target = "_blank";
      }
    ];
    # stashvr is on cirdan, but points to william, so we use william's enabling for if it's active
    stashvr = lib.lists.optionals config.services.stashapp.enable [
      {
        name = "Stash Data Headset";
        icon = "fas fa-vr-cardboard";
        # Service is ONLY accessible from william, so hitting it with a port won't work unless you ARE william
    ...
    sickgear = lib.lists.optionals config.services.sickbeard.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString config.services.sickbeard.port}/tv/"
          else
    ...
    radarr = lib.lists.optionals config.services.radarr.enable [
      {
        ...
        url =
          if internal then
            "http://${configVars.networking.subnets.william.ip}:${builtins.toString configVars.networking.ports.tcp.radarr}/"
          else
