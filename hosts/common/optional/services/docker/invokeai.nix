...
let
  ...
  # Create an invokeai.yaml config file for model search paths
  invokeConfigFile = pkgs.writeText "invokeai.yaml" ''
    # InvokeAI Configuration
    # Auto-generated by NixOS
    schema_version: "4.0.0"

    # Scan these directories for models
    model_search_paths:
      - /invokeai/models/shared/checkpoints
      - /invokeai/models/shared/loras
      - /invokeai/models/shared/vae
      - /invokeai/models/shared/controlnet
      - /invokeai/models/shared/text_encoders
      - /invokeai/models/shared/diffusion_models
      - /invokeai/models/shared/unet
      - /invokeai/models/shared/clip_vision
      - /invokeai/models/shared/embeddings
      - /invokeai/models/shared/upscale_models
  '';
in
{
  imports = [
  ...
  options.services.invokeai = with types; {
    ...
    gpuType = mkOption {
      ...
      description = ''
        ...
        - "nvidia": Uses NVIDIA GPU with CUDA
        - "amd-rocm": Uses AMD GPU with ROCm
  ...
  config = mkMerge [
    # Auto-detect GPU type from nixpkgs config (like ComfyUI does)
    (mkIf cfg.enable {
      services.invokeai.gpuType = mkDefault (
        if config.nixpkgs.config.rocmSupport or false then "amd-rocm" else "nvidia"
      );
    })

    # Main InvokeAI configuration
    (mkIf cfg.enable {
        ...
        services."invokeai".service = mkIf cfg.active (mkMerge [
          # Base configuration
          {
            ...
            environment = {
              ...
              # Add shared model directories to search paths
              INVOKEAI_MODEL_SEARCH_PATHS = "/invokeai/models/shared/checkpoints:/invokeai/models/shared/loras:/invokeai/models/shared/vae:/invokeai/models/shared/controlnet:/invokeai/models/shared/text_encoders:/invokeai/models/shared/diffusion_models:/invokeai/models/shared/unet:/invokeai/models/shared/clip_vision:/invokeai/models/shared/embeddings:/invokeai/models/shared/upscale_models";
            }
            // (lib.optionalAttrs (cfg.gpuType == "amd-rocm") {
            ...
            # Add symlinked model directories if symlinkPaths is configured
            ++ (lib.optionals (config.services.comfyui.symlinkPaths != { }) (
              lib.mapAttrsToList (
                name: hostPath: "${hostPath}:/invokeai/models/shared/${name}:ro"
            ...
            devices =
              if cfg.gpuType == "nvidia" then
                [ "nvidia.com/gpu=all" ]
              else if cfg.gpuType == "amd-rocm" then
                [
                  "/dev/kfd:/dev/kfd"
                  "/dev/dri:/dev/dri"
                ]
          ...
          # ROCm-specific: Build custom image for gfx1151
          (mkIf (cfg.gpuType == "amd-rocm") (
            let
              # Create a Dockerfile that extends the official ROCm image with gfx1151 PyTorch
              rocmDockerfile = pkgs.writeText "Dockerfile.invokeai-rocm" ''
                FROM ghcr.io/invoke-ai/invokeai:v${cfg.version}-rocm

                # Switch to root for package management
                USER root

                # Reinstall PyTorch with gfx1151 (Strix Halo) support
                # InvokeAI uses uv for package management
                ...
                # Don't switch user - let the entrypoint handle it
              '';

              buildContext = pkgs.runCommand "invokeai-rocm-context" { } ''
                mkdir -p $out
          ...
          # NVIDIA: Use official CUDA image
    ...
    (mkIf (cfg.enable && cfg.gpuType == "nvidia") {
      hardware.nvidia-container-toolkit.enable = mkDefault true;
    })

    # AMD ROCm-specific configuration
    (mkIf (cfg.enable && cfg.gpuType == "amd-rocm") {
      # Ensure user has access to render/video groups for ROCm
      # This will need to be set per-user basis in the host config
    })
  ];
}
