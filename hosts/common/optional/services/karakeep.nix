...
}:
{
  ...
  sops.templates."karakeep-secrets.env" = {
    content = ''
      ...
      SMTP_PORT=${config.sops.placeholder."homelab/smtp/host"}
      SMTP_SECURE=${config.sops.placeholder."homelab/smtp/ssl"}
      SMTP_USER=${config.sops.placeholder."homelab/smtp/host"}
      SMTP_PASSWORD=${config.sops.placeholder."homelab/smtp/host"}
  ...
  services.karakeep.extraEnvironment.DISABLE_SIGNUPS = "true";
  ...
  services.karakeep.extraEnvironment.OAUTH_PROVIDER_NAME = "authenthik";
