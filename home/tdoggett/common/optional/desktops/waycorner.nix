{ pkgs, lib, ... }:
{
  ...
  services.waycorner.settings.main-monitor.enter_command = lib.mkDefault [ "${pkgs.hyprland}/bin/hyprctl" "dispatch" "overview:toggle" ];
  # services.waycorner.settings.main-monitor.exit_command = lib.mkDefault [ "notify-send" "exit" ];
  services.waycorner.settings.main-monitor.locations = lib.mkDefault [ "bottom_right" "bottom_left" ];
  ...
  services.waycorner.settings.side-monitor.enter_command = lib.mkDefault [ "${pkgs.hyprland}/bin/hyprctl" "dispatch" "overview:toggle" ];
  # services.waycorner.settings.side-monitor.exit_command = lib.mkDefault [ "notify-send" "exit" ];
  services.waycorner.settings.side-monitor.locations = lib.mkDefault [ "bottom_right" "bottom_left" ];
